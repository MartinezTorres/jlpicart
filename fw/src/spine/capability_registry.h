...
// capability_registry.h — Declared → Allowed capability pipeline (Stage 3).
//
// The registry owns the three-stage capability lifecycle:
//   Declared: board declares HW / driver table declares SW
//   Allowed:  declared ∩ (not masked by policy)
//   Activated: not implemented until Stage 8
//
...
struct CapabilityEntry {
    const char* name;
    bool        is_hw;      // true = from board descriptor; false = SW driver
    bool        safe_verify;
...
public:
    ...
    // Computes the Allowed set by applying policy masking rules.
    void init(const BoardDescriptor&  board,
              const DriverDescriptor* drivers,
              size_t                  driver_count,
              const PolicyInfo&       policy);

    // Queries — O(n) linear scan; registry is small and called rarely.
    bool is_declared(const char* name) const;
    bool is_allowed (const char* name) const;

    // List all declared or allowed names (up to `max` entries).
...
private:
    ...
    // Currently no capability names are policy-gated in Stage 3 (masking
    // will be populated as features are added in Stage 9+).
